import { generateDeepSeekJson, mapDeepSeekError, parseModelJson } from "@/lib/ai/deepseek";
...
export class DeepSeekSubmissionIntelligenceProvider implements SubmissionIntelligenceProvider {
  ...
  async generateCoachBrief(input: CoachBriefInput): Promise<CoachBriefPayload> {
    ...
    for (let attempt = 0; attempt <= BRIEF_LIMITS.maxAutoRetries; attempt += 1) {
      try {
        const text = await generateDeepSeekJson(SYSTEM_PROMPT, input, getCoachBriefModel());
...
function mapIntelligenceError(error: unknown): IntelligenceProviderError {
  ...
  return new IntelligenceProviderError(mapDeepSeekError(error).code);
}

export function createSubmissionIntelligenceProvider(schema: OnboardingSchema): SubmissionIntelligenceProvider {
  return new DeepSeekSubmissionIntelligenceProvider(schema);
}
